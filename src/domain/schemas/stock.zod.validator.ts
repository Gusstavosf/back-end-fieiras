...
export const estoqueSchema = z.object({
  cabinetName: z.string({ message: "O campo armário é obrigatório e deve estar entre CTC000..CTC999 ou WM000..WM999" })
  .regex(/^(CTC\d{3}|WM\d{3})$/, {
      message: "O campo armário é obrigatório e deve estar entre CTC000..CTC999 ou WM000..WM999"
    }),

  code: z.string({ message: "O campo código da fieira é obrigatório e deve estar entre A00..Z99." })
  .regex(/^[A-Z]\d{2}$/, {
    message: "O campo código da fieira é obrigatório e deve estar entre A00..Z99."}),

  status: z.enum(['new', 'requested', 'dead', 'polished'], { 
    message: "O campo status é obrigatório e deve ser: new, requested, dead ou polished."}),
